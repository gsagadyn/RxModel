...
extension PrimitiveSequence where TraitType == MaybeTrait {
    public func cached<T>(forType type: T, expiry: Expiry = .never) -> Maybe<ElementType> {
    ...
    public func cached<T, S: Storage>(on storage: S, forType type: T, expiry: Expiry = .never) -> Maybe<ElementType> where S.Key == String {
    ...
    public func removeCachedValue<T>(forType type: T) -> Maybe<ElementType> {
    ...
    public func removeCachedValue<T, S: Storage>(from storage: S, forType type: T) -> Maybe<ElementType> where S.Key == String {
